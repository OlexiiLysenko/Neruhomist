			...
			if (get_custom_option('show_twitter_in_footer') == 'yes'  ) {
				...
				if ($twitter) {	?>
					<div class="twitBlockWrap"><span class="twitterTitle"><?php echo __('Latest Tweet Update', 'themerex'); ?></span><?php echo balanceTags($twitter); ?>
						<span class="twitterAuthor"><?php echo __('Follow us on', 'themerex').' <a href="'.themerex_get_protocol().'://twitter.com/' . get_theme_option('twitter_username') . '" target="_blank">' . __('Twitter', 'themerex') . '</a> '.__('for our latest updates', 'themerex'); ?></span>
					</div>
					<?php
				}
			}
				...
				if ( get_custom_option('googlemap_show')=='yes' ) {
					...
					if (!empty($map_address) || !empty($map_latlng)) {
						echo do_shortcode('[trx_googlemap'
							. (!empty($map_address) ? ' description="'.$map_address.'"' : '')
