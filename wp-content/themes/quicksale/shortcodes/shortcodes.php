...
function sc_countdown($atts, $content = null) {
	...
	$output = '<div '.($id ? ' id="'.$id.'"' : '').'class="sc_countdown '.$c.'"'.($s ? ' style="'.$s.'"' : '').'><div class="sc_countdown_counter" data-style="'.($style == 'flip' ? 'flip' : 'square').'" data-date="'.$date.'" data-time="'.$time.'"></div></div>';
...
function sc_emailer($atts, $content = null) {
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation sFocus' . ($align && $align!='none' ? ' sc_align' . $align : '') . '' . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
...
function sc_gap_wrapper($str) {
	...
	if ($str_new != $str) $str_new = preg_replace('/('.sc_gap_end().')[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)[\r\n\s]*(<\/div>)/i', '\\2\\3\\4\\1', $str_new);
	// New VC wrappers
...
function sc_image($atts, $content=null){	
	...
	if(sc_param_is_on($link) && !empty($src) && !empty($url_color)){
		...
		$output = ('<a href="'.esc_url(trim($href)).'"' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image sc_image_hover_link' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
