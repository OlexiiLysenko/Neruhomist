...
if(!function_exists('ct_wp_footer')) {
	function ct_wp_footer() {
		...
		if($ct_search_results_listing_style == 'list' || $ct_header_currency_switcher == 'yes') { ?>
		<script>
	    jQuery(window).load(function($) {
	    	...
	    	foreach($response->businesses as $business) {
			    ...
			    echo '<li>';
			    	...
			    	echo '<div class="col span_3">';
				    	 echo '<img class="left" src="' . $response->businesses[$i]->rating_img_url . '" /> <span class="review-count muted right">' . $response->businesses[$i]->review_count . ' ' . __('reviews', 'contempo') . '</span></a>';
				    echo '</div>';
				    	echo '<div class="clear"></div>';
			    echo '</li>';
			    $i++;
